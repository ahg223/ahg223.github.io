...
* Endorsing Peer: 스마트 컨트랙트에 수행될 트랜잭션을 시뮬레이션해보고 그 결과를 클라이언트 어플리케이션에 리턴해주는 피어다. 트랜잭션을 검증하는 역할을 한다.
* Committing Peer: 오더링까지 되어서 전달된 트랜잭션 블럭을 검증하고, 검증이 완료되면 자기가 갖고 있는 원장에 해당 블럭을 추가하고 관리한다. 모든 피어는 기본적으로 committing peer다.
* Anchor Peer: 채널 대표 피어다. 채널에 다른 조직이 가입하면 가장 먼저 발견하는 피어.
* Leading Peer: 여러 피어를 가지고 있는 조직의 경우, 모든 피어를 대표하여 네트워크와 커뮤니케이션 하는 피어.

### Assets

블록체인은 거래를 전제로 하는 네트워크다. 그 거래가 이루어질려면 부동산과 같은 유형에서 계약, 지적 재산과 같은 품목을 Assets이라고 한다. Assets은 패브릭에서 키 값 쌍으로 표시되며 상태 변경은 채널 Ledger에 트랜잭션으로 기록된다.
 
즉, 거래의 과정에 따라 Asset에 *State*가 생기며 이 State가 변하는 것을 기록한것이 *Ledger*이다.
 
### Chaincode

Chaincode는 거래 지시 사항이다. Chaincode키 값 쌍 또는 다른 상태 데이터베이스 정보를 읽거나 변경하기 위한 규칙을 시행한다. Chaincode기능은 원장의 상태 데이터베이스에 대해 실행되며 거래 제안을 통해 시작된다. 체인코드 실행은 네트워크에 제출되어 모든 피어의 원장에 키 값이 적용된다.
 
네트워크 권한에 따라 접근할 수 있는 Chaincode가 달라진다. 여러 트랜잭션이 동시에 발생한다면 그 발생한 순서에 따라 제대로 ordering 될 때만 실제 ledger에 반영이 된다.
 
### Ledger

Ledger는 패브릭의 모든 상태 변경 기록과 상태이다. 상태 변경은 참여자가 제출 한 Chaincode의 결과이다. Ledger는 이러한 결과가 적힌 트랜젝션의 합을 블록으로 저장하는 블록체인과 현재 패브릭 상태를 유지하는 스테이트 데이터베이스로 구성된다. 채널당 1개의 원장이 있으며 각 피어는 각 채널에 대해 원장 사본을 보관한다.
 
### Consensus

Consensus는 합의다. 블록체인에서 합의란, 모든 참여자들이 동의하는 과정을 뜻한다. 즉, 블록체인 네트워크 내의 peer들이 항상 동기화 된 State DB와 Ledger를 가지게 하는 기술이다.  
하이퍼래저 페브릭에서 합의는 단순히 거래 순서에 동의하는 것 이상의 의미를 지니는데, 제안 및 보증, 오더링, 검증에 이르기까지 전체 거래 흐름에서 합의가 필요하다. 합의는 블록을 구성하는 일련의 트랜잭션의 정확성에 대한 완전한 단일 검증으로 정의된다. 컨센서스는 블록 트랜잭션의 순서와 결과가 명시적인 기준 검사를 충족하면 달성된다. 이러한 확인 및 잔액은 거래의 주기 동안 발생하며 특정 거래 클래스를 보증해야하는 특정 회원 및 시스템 Chaincode를 보증하는 보증정책의 사용을 포함하여 이러한 정책이 시행되고 유지되도록 한다.
